...
def pointnet_pyramid_net_module(xyz, num_scale, points, npoints, radius, nsample, mlp, group_all, is_training,
                       bn_decay, scope='irregular_pyramid', bn=True, ibn=False, pooling='max', tnet_spec=None, knn=False, use_xyz=True):
    ''' PointNet Set Abstraction (SA) Module
        ...
        Return:
            new_xyz: (batch_size, npoint, 3) TF tensor
            new_points: (batch_size, npoint, mlp[-1] or mlp[-1]) TF tensor
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for s in range(num_scale):
            ...
            new_xyz, new_points, idx, grouped_xyz = sample_and_group(npoints[s], radius[s], nsample, xyz, points, tnet_spec, knn, use_xyz)
            ...
            for i, num_out_channel in enumerate(mlp):
                new_points = tf_util.conv2d(new_points, num_out_channel, [1,1],
                                            ...
                                            scope='scale_'+str(s)+'_conv%d'%(i), bn_decay=bn_decay)
                new_points = tf.layers.batch_normalization(new_points, name='batch_norm_'+str(s)+'_conv%d'%(i))
            ...
            up_points = pointnet_fp_module(xyz, new_xyz, points, new_points, [], is_training, bn_decay,
                                       scope='scale_'+str(s)+'fa_layer',bn=bn,ibn = ibn)

            pyramid_features.append(up_points)

        pyramid_features = tf.concat(pyramid_features, axis = 2)
...
def pointnet_sa_module_msg(xyz, points, npoint, radius_list, nsample_list, mlp_list, is_training, bn_decay, scope, bn=True, ibn = False, use_xyz=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        for i in range(len(radius_list)):
            ...
            if points is not None:
                grouped_points = group_point(points, idx) # # (batch_size, npoint, channel)
                if use_xyz:
                    grouped_points = tf.concat([grouped_points, grouped_xyz], axis=-1) # (batch_size, npoint, channel+3)
            else:
